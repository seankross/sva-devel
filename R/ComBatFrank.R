...
ComBatFrank <- function(dat, batch, mod=NULL, par.prior=TRUE,prior.plots=FALSE) {
  ...
  # New C++
  delta.hat <- calculateDeltaHat(s.data, batches)
  ...
  if(par.prior){
    ...
    # New C++
    gammaDeltaStar <- parametricAdjust(s.data, gamma.hat, delta.hat, gamma.bar, t2, a.prior, b.prior,batches, n.batch)
  }else{
    ...
    # New C++
    gammaDeltaStar <- nonparametricAdjust(s.data, gamma.hat, delta.hat, batches, n.batch)
  }
